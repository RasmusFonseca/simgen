...
static GLfloat  dout = Data::viewat; // eye position on Z
...
float	cthru, range = 0.00001,
	...
	if (cell->heat > 0.1) {
		cell->solid = -999;     // always see into an active object
		return 1.0 - 0.7*cell->heat; // 0.3-->1 as heat drops (0.3 at full heat)
	}
        if (type <= 0) {
	...
	// angle condition
        ...
        if (level==depth) cthru = 1.0; // always solid
...
float	d;
	...
	if (Data::frozen < 0) { // track specified focus (slowly)
		eye.x = focus.x; eye.y = focus.y; eye.z = trZ-dout;
	} else {
