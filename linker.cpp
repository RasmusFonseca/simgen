...
void	fixHinge ( Cell*, Bonds*, float, float, float, int );
void	fixHinge ( Cell*, Bonds*, float, float, float ); //       ntry=10
void	fixHinge ( Cell*, Bonds*, float, float ); //     creep=1, ntry=10
void	fixHinge ( Cell*, Bonds*, float ); // limit=0.1, creep=1, ntry=10
...
int	end = 0;
	if (cell==0) return;
	if (cell->busy || cell->empty) return;
	n = cell->kids;
			...
			if (level==depth-2 && cell->type==2) {	// bond domain tube ends with 0 length bond
				...
				if (subtype==1) { Vec v; float w = 0.1;	// link DNA (but don't join RNA ends)
					if (cell->sis->level==level) { Vec Nter, Cter; // fix sis side
						v = (cell->sis->endC - cell->sis->endN).getNorm();
						Cter = cell->sis->xyz + v*(cell->sis->len*0.6);
						v = (cell->endN - cell->endC).getNorm();
	...
	if (nbonds > 1) { // may be cross-linked
        ...
        if (Data::frozen && cell->heat < randf()) return;
...
//	if (level && cell->parent->solid > 0) return; 
	if (cell->link) { float d;
		...
		FOR(i,nlinks) { Cell *link = cell->link[i].to; int type, brittle; float snap;
			if (link==0) continue;
			if (link->busy || link->empty) continue;
...
#define TRIES 100
#define CREEP 0.5 // move in slowly towards target
#define LIMIT 0.1 // limit maximum rotation angle

void fixHinge ( Cell *at, Bonds *bond, float leng )
{
	fixHinge(at,bond,leng,LIMIT,CREEP,TRIES);
}

void fixHinge ( Cell *at, Bonds *bond, float leng, float limit )
{
	fixHinge(at,bond,leng,limit,CREEP,TRIES);
}

void fixHinge ( Cell *at, Bonds *bond, float leng, float limit, float creep )
{
	fixHinge(at,bond,leng,limit,creep,TRIES);
}
...
void fixHinge ( Cell *at, Bonds *bond, float leng, float limit, float creep, int ntry )
{
  ...
  Cell	*to = bond->to;
	...
	// Pt(hold bc) Pr(creep) Pi(link) Pr(ab) Pr(bc) Pr(cd) Pr(ad) NL
	if (ad > ab+leng+cd) { float dist; // make colinear
		ta = angle(b,a,d)*creep;
		if (ta > NOISE) {
			if (ta>limit) ta = limit;
			x = (b-a).getNorm();
			y = (d-a).getNorm();
			z = x^y; g = a+z;
			slow_spinCell(at,a,g,-ta);
		}
		td = angle(c,d,a)*creep;
		if (td>NOISE) {
			if (td>limit) td = limit;
			x = (c-d).getNorm();
			y = (a-d).getNorm();
			z = x^y; g = d+z;
			slow_spinCell(to,d,g,-td);
		}
		dist = leng+(ab+cd)*0.5; // mid-stick separation
		part2cells(at,to,dist,creep);
		return;
	}
	// t sets the size of the random displacement of the ends
	t =  leng-bc; if (t<0.0) t = -t;
	t = sqrt(t);	// make t bigger for bigger gaps
	t *= 0.1;
	if (t<NOISE) return;
	ta = t*sqrt(ab);	// make t bigger for
	td = t*sqrt(cd);	// longer sticks
	m = n = 1;
	bs[0] = b; cs[0] = c; // 1st try (bs[0],cs[0]) = current position
	best = scoreFit(a,b,c,d,b,c,leng);
	FOR(i,100) { float len2 = leng*2.0;;
		if (n==ntry && m==ntry) break;
		e = rdisp(a,bs[n-1],ta);
		f = rdisp(d,cs[m-1],td);
		score = scoreFit(a,b,c,d,e,f,leng);
		if (score > best) continue;
		in = 0;
		if (n<ntry && (e|d)>cd) { // e is outside D
			bs[n] = e;
			n++; in++;
		}
		if (m<ntry && vdif(f,a)>ab) { // f is outside A
			cs[m] = f;
			m++; in++;
		}
		if (in==2) best = score;
